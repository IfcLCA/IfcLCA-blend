...
def get_database_reader(db_type: str, db_path: str = None) -> IfcLCADBReader:
    """Factory function to get appropriate database reader.
    
    Args:
        db_type: Type of database ('KBOB', 'CUSTOM')
        db_path: Path to database file (optional for KBOB, required for others)
        
    Returns:
        Instance of appropriate database reader
        
    Raises:
        ValueError: For unknown database type
    """
    if db_type == "KBOB":
        ...
        # Assume JSON format for custom databases
        return KBOBReader(db_path)
    else:
